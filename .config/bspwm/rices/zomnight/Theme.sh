...
# Set bspwm configuration for zomnight
set_bspwm_config() {
...
set_term_config() {
		...
		-e "s/colors: .*/colors: *zombie_night/"
}
...
# Launch the bar
launch_bars() {
		polybar -q zomnight-bar -c ${rice_dir}/config.ini &
		polybar -q zomnight-bar2 -c ${rice_dir}/config.ini &
		polybar -q zomnight-bar3 -c ${rice_dir}/config.ini &
}
